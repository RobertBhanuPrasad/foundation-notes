...
(defn add-todo [{:keys [:biff.xtdb/node params]}]
  (let [title (:title params)
        id    (str (random-uuid))
        todo  {:xt/id id :todo/title title}] 
    ...
    [:li
     ...
     [:span.title {:style {:flex "1" :cursor "pointer"}
                   :hx-get  (str "/todo/todos/" id)
                   :hx-target (str "#todo-" id)
                   :hx-swap "outerHTML"} (:todo/title todo)]
     ;;  [:button#edit-button {:type "button" :class "btn" :hx-get "todo/todos" :hx-target "previous" :hx-swap "outerHTML"} "Edit"]
     [:button {:type "button" :class "btn danger"
               :hx-target (str "#todo-" id)
               :hx-swap "delete"
...
(defn edit-todo [{:keys [:biff.xtdb/node path-params]}]
  (println "hiturleditbhanu" path-params)
  (let [id (:id path-params)
        todo (xt/entity (xt/db node) id)]
    (println "todoeditbhanu" todo)
    (if todo
      [:li
       {:id (str "todo-" id)
        :style {:display "flex" :align-items "center"
...
(defn update-todo [{:keys [:biff.xtdb/node path-params params]}]
  (let [id (:id path-params)
        title (:title params)
        todo {:xt/id id :todo/title title}]
    (xt/submit-tx node [[::xt/put todo]])
...
(defn remove-todo [{:keys [:biff.xtdb/node path-params]}]
  (let [id (:id path-params)]
    (xt/submit-tx node [[::xt/delete id]])
    (xt/sync node)
    {:status 200}))

(defn all-todos [{:keys [:biff.xtdb/node]}]
  (let [db (xt/db node)
    todos (xt/q db '{:find [id title]
                     :where [[id :todo/title title]]})
        todos (map (fn [[id title]] {:id (str id) :title title}) todos)]
    (println todos "todosbhanu")
    [:ul#todo-list
     (for [{:keys [id title]} todos]
       ^{:key id}
       [:li
